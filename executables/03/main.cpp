...
int main(int argc, char* argv[]) {
    // -----------------------------------------------------------------
    // Milestone 03: configurable relaxation parameter tau.
    // Optional CLI override:  ./milestone03 <tau>
    // omega = 1/tau must satisfy 0 < omega < 2  =>  tau > 0.5 for stability.
    // We only accept argv[1] as tau if it parses to a positive number and
    // does not look like a flag (so Kokkos-prefixed args are left alone).
    // -----------------------------------------------------------------
    double tau = 1.0; // default relaxation time (omega = 1.0)
    if (argc > 1) {
        std::string arg1(argv[1]);
        if (!arg1.empty() && arg1[0] != '-') {
            double parsed = std::atof(argv[1]);
            if (parsed > 0.0) {
                tau = parsed;
            }
        }
    ...
    Kokkos::initialize(argc, argv);
    {
        const int NX = 15;
        const int NY = 10;
        const int total_steps = 50; // Set to observe multiple periodic wrap-arounds
        ...
        // D2Q9 Lattice Weights (Milestone 03): w0=4/9, w1-w4=1/9, w5-w8=1/36
        ...
        // --- Initialization: Omnidirectional distribution seeding (unchanged from Milestone 02) ---
        const double rho_background = 1.0;
        const double rho_bump       = 1.1;

        for (int y = 0; y < NY; ++y) {
            for (int x = 0; x < NX; ++x) {

                double rho_init = rho_background;

                if (x == NX/2 && y == NY/2)
                    rho_init = rho_bump;

                for (int i = 0; i < NVEL; ++i) {
                    f_host(y,x,i) = h_w[i] * rho_init;
                }
            }
        }
        ...
        for (int step = 0; step <= total_steps; ++step) {
            ...
            std::ofstream outFile("lbm_step_" + std::to_string(step) + ".txt");
    ...
    Kokkos::finalize();
    std::cout << "Successfully executed BGK collision and streaming for Milestone 03." << std::endl;
    return 0;
}