...
GHIA = {
    100: {
        ...
        "ux": [0.00000,-0.03717,-0.04192,-0.04775,-0.06434,-0.10150,-0.15662,
    ...
    400: {
        "y":  [0.0000,0.0547,0.0625,0.0703,0.1016,0.1719,0.2813,
               0.4531,0.5000,0.6172,0.7344,0.8516,0.9531,0.9609,
               0.9688,0.9766,1.0000],
        "ux": [0.00000,-0.08186,-0.09266,-0.10338,-0.14612,-0.24299,-0.32726,
    ...
    1000: {
        "y":  [0.0000,0.0547,0.0625,0.0703,0.1016,0.1719,0.2813,
               0.4531,0.5000,0.6172,0.7344,0.8516,0.9531,0.9609,
               0.9688,0.9766,1.0000],
        "ux": [0.00000,-0.18109,-0.20196,-0.22220,-0.29730,-0.38289,-0.27805,
...
def _add_lid_arrow(ax):
    ax.annotate("", xy=(0.85, 1.015), xytext=(0.55, 1.015),
...
def _streamplot_frame(ax, x1d, y1d, ux, uy, norm, title_str):
    ...
    ax.set_title(title_str, fontsize=10, pad=22)
    _add_lid_arrow(ax)
    return strm
...
def main():
    ...
    if os.path.exists(args.field):
        try:
            ...
            REAL_RE = 0.1 * 256 / (1/3 * (1/1.7 - 0.5))   # = 870.4, from your actual OMEGA/U_LID/NX
            plot_streamlines(ux, uy, NX, NY, re=REAL_RE)
        except Exception as e:
            print(f"Error: {e}")
    else:
